...
// 找出view上的最后一个HUD，若存在则隐藏并返回YES，否则NO
...
- (void)show:(BOOL)animated {
    ...
    // 设置graceTime属性，防止网络请求很快返回时也显示转圈控件，需要配合taskInProgress属性才有效，具体看代码
...
// 隐藏动画完成后做的清理工作，通知代理，调用block，删除视图等
- (void)done {
	[NSObject cancelPreviousPerformRequestsWithTarget:self];
	isFinished = YES;
	self.alpha = 0.0f;
...
- (void)layoutSubviews {
	...
	// HUD的尺寸等于父视图的尺寸，故用户无法与HUD的父视图进行交互
